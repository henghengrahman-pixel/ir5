...
function uploaded(req, field){
  return req.files?.[field]?.[0]?.filename ? `/uploads/${req.files[field][0].filename}` : '';
}
function thumbnailFromRequest(req, fallback = ""){
  return uploaded(req,'thumbnailFile') || req.file?.filename && `/uploads/${req.file.filename}` || req.body.thumbnailUrl?.trim() || fallback || "";
}
function coverFromRequest(req, fallback = ""){
  return uploaded(req,'coverFile') || req.body.coverUrl?.trim() || fallback || "";
}
...
export async function settingsUpdate(req,res){ await saveSettings({
  siteName:req.body.siteName?.trim(), logoUrl:uploaded(req,'logoFile') || req.body.logoUrl?.trim(), faviconUrl:uploaded(req,'faviconFile') || req.body.faviconUrl?.trim(), backgroundDesktop:uploaded(req,'backgroundDesktopFile') || req.body.backgroundDesktop?.trim(), backgroundMobile:uploaded(req,'backgroundMobileFile') || req.body.backgroundMobile?.trim(), ogImage:uploaded(req,'ogImageFile') || req.body.ogImage?.trim(), primaryColor:req.body.primaryColor?.trim(), accentColor:req.body.accentColor?.trim(), loginUrl:req.body.loginUrl?.trim(), registerUrl:req.body.registerUrl?.trim(), telegramUrl:req.body.telegramUrl?.trim(), whatsappUrl:req.body.whatsappUrl?.trim(), livechatUrl:req.body.livechatUrl?.trim(), runningText:req.body.runningText?.trim(), footerText:req.body.footerText?.trim(), footerHtml:req.body.footerHtml?.trim(), metaTitle:req.body.metaTitle?.trim(), metaDescription:req.body.metaDescription?.trim(), metaKeywords:req.body.metaKeywords?.trim()
...
export async function postCreate(req,res){
  ...
  posts.push({
    ...
    type:req.body.type?.trim() || (parsePredictions(req.body.predictions).length ? 'prediction' : 'article'),
    category:req.body.category?.trim() || 'Berita Bola',
    tags:splitTags(req.body.tags),
    author:req.body.author?.trim() || 'Master Parlay',
    thumbnail:thumbnailFromRequest(req),
    cover:coverFromRequest(req),
...
export async function postUpdate(req,res){
  ...
  await savePosts(posts.map(item => item.id === current.id ? {
    ...item,
    title, slug,
    type:req.body.type?.trim() || item.type || (parsePredictions(req.body.predictions).length ? 'prediction' : 'article'),
    category:req.body.category?.trim() || item.category || 'Berita Bola',
    tags:splitTags(req.body.tags),
    author:req.body.author?.trim() || item.author || 'Master Parlay',
    thumbnail:thumbnailFromRequest(req, item.thumbnail),
    cover:coverFromRequest(req, item.cover),
    featured:toBool(req.body.featured),
    trending:toBool(req.body.trending),
    excerpt:req.body.excerpt?.trim() || makeExcerpt(content),
    content,
