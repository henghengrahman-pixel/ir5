...
function isExpiredPredictionPost(post){
  if (!post?.autoParlay && post?.type !== 'prediction') return false;
...
export async function cleanupOldPredictions(){
  ...
  const filtered = rows.filter(post => !isExpiredPredictionPost(post));
...
export async function savePosts(rows){
  const cleaned = [...rows].filter(post => !isExpiredPredictionPost(post)).sort(byDateDesc);
